...
Minimální EC2 instance (`t3.micro`, Amazon Linux 2023) ve vlastní,
izolované VPC (ne default VPC účtu — viz ADR-005), s otevřeným
...
| 2026-09-05 | Fáze B zahájena: honeypot EC2 (`t3.micro`, Amazon Linux 2023, žádný SSH klíč, žádná IAM role) nasazen ve Workload účtu do default VPC, veřejná IP `18.205.237.67` přiřazena, port 22 otevřený pro internet. Launch time: 2026-09-05 14:44 CEST |
| 2026-09-05 | Rozhodnuto přejít z default VPC na vlastní izolovanou VPC (viz ADR-005). Terraform rozšířen o `network.tf` (VPC, veřejný subnet, Internet Gateway, route table); honeypot instance a security group zrekreovány (`terraform apply`: 7 added, 2 destroyed). **Nová veřejná IP: `34.235.151.169`**, VPC `vpc-0d0fc23791e16c800`. Auto-shutdown timer se restartoval s novou instancí — naplánován na **2026-09-08** (~14:5x CEST, dle přesného launch time nové instance). Čeká se na první reálné GuardDuty nálezy |
| 2026-09-05 | EventBridge rule zúžena jen na GuardDuty-origin findings (`ProductFields."aws/securityhub/ProductName": ["GuardDuty"]`) po incidentu s 200+ e-maily z plošného Security Hub CSPM přepočtu (Informational nálezy napříč nepoužívanými službami — CloudFront, RDS, MSK, atd.) |
...
| 2026-09-08 | **Fáze B uzavřena.** Honeypot přešel do OS-level auto-shutdown (stopped) po ~3 dnech provozu. 24hodinová Flow Log data: `51.15.25.116` s 1 600+ pokusy o port 22, pravidelný denní vzorec aktivity (~20:00, typický cron-like automatizovaný nástroj) — i přesto **žádný GuardDuty finding**. Finální závěr zdokumentován v ADR-007: GuardDuty threat-intel korelace jako určující faktor detekce se potvrdila i při dlouhodobém pozorování; VPC Flow Logs s vlastní analýzou odhalily útok, který GuardDuty přehlédla — argument pro doplňkovou detekční vrstvu v produkčním nasazení |
| 2026-09-08 | Externí potvrzení: AbuseIPDB kontrola `51.15.25.116` ukázala 100% confidence of abuse, 48 reportů od 31 nezávislých zdrojů (Scaleway VM, ASN AS12876), včetně citace jiného, nezávislého honeypotu hlásícího stejnou IP za stejný typ útoku — nezávislé potvrzení vlastní detekční metodiky |
